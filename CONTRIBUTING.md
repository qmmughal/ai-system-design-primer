...
1. Write a complete chapter in `docs/topics/`. The file must teach: a definition,
   a diagram, numbers, failure modes, and a "what interviewers listen for"
   section. No stubs.
2. Link it from the [README index](README.md) and from `mkdocs.yml`.
3. If the topic changes interview advice, update the relevant interview.

## How to add a failure

Use the template in [`docs/failures/README.md`](docs/failures/README.md). One file per
failure. Title the file after the *mechanism* (`rag-silent-lies.md`), not the
symptom (`wrong-answers.md`).

## How to add an interview

Use the template in [`docs/interviews/README.md`](docs/interviews/README.md). Solutions
...
Prose is [CC BY-SA 4.0](LICENSE). Code samples are MIT. By contributing you
